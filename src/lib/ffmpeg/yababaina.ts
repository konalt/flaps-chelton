...
export default async function yababaina(images: Buffer[]) {
    ...
    let final = await ffmpegBuffer(
        [
            ...
            `-pattern_type sequence -r ${fps} -loop 1 -f image2 -i http://localhost:56033/${backgroundReference}`,
            `-pattern_type sequence -r ${fps} -loop 1 -f image2 -i http://localhost:56033/${spinAnimationReference}`,
            `-pattern_type sequence -r ${fps} -loop 1 -f image2 -i http://localhost:56033/${acrossSpinAnimationReference}`,
            `-pattern_type sequence -r ${fps} -loop 1 -f image2 -i http://localhost:56033/${trioAnimationReference}`,
            `-pattern_type sequence -r ${fps} -loop 1 -f image2 -i http://localhost:56033/${lyricsReference}`,
            "-multiple_requests 1",
            "-seekable 1",
            "-reconnect_at_eof 1",
            "-v verbose",
            `-filter_complex "[2:v]split=2[spin_l][spin_r_noflip];[spin_r_noflip]hflip[spin_r];`,
            `[4:v]split=3[trio_main_ns][trio_big_noscale][trio_big2_noscale];[trio_big_noscale]scale=2.6*iw:2.6*ih[trio_big];[trio_big2_noscale]scale=1.3*iw:2.4*ih[trio_big2];`,
            `[trio_main_ns]scale='${trioMainScaleFunc}*iw:${trioMainScaleFunc}*ih:eval=frame'[trio_main];`,
            "[1:v][trio_main]overlay='W/2-w/2:H/2-h/2:eval=frame:enable=not(between(t\\,3.3\\,4.9))'[o_toacross];",
            overlayFilter("o_toacross", "3:v", "o_tospins", 0, 0, w, h),
            ";",
            overlayFilter(
                "o_tospins",
                "spin_l",
            ...
            `;[o_tb2][5:v]overlay=0:H-h[out_final]`,
            `;[0:a]anull[out_audio]"`,
