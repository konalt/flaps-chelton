...
function createScoreVideo(input, output, options) {
    return ffmpeg(
        `${
            verbose ? "" : "-v warning "
        }-y -loop 1 -t ${segmentLength} -i ${input}.png -filter_complex "[0:v]scale=-1:800,crop=400:800,setsar=1:1,setpts=PTS-STARTPTS[scaled];[scaled]drawtext=text='${
            options.curScore
        }':x=W/2-tw/2:y=H/2-th/2:fontsize=72:fontfile=font.ttf:borderw=5:bordercolor=black:fontcolor=white,fps=25[out]" -map "[out]" -t ${segmentLength} -r 25 ${output}.mp4`
    );
}
...
async function todo(client, msg) {
    ...
    if (msg.attachments.first(2)[1]) {
        ...
        download(att[0].url, path.join(__dirname, "img1.png"), async() => {
            console.log("downloaded image 1");
            download(att[1].url, path.join(__dirname, "img2.png"), async() => {
                var inputs = ["versus/img1", "versus/img2"];
                var segmentLengths = [70 /* , 126 / 2, 80 */ ];
                ...
                await createWinnerVideo(inputs, "versus/" + vi++, names);
