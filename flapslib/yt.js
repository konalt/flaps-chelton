...
function getFirstResultID(search) {
    var ytProcess = cp.spawnSync("yt-dlp", ("ytsearch:" + search.split(" ").join("+") + " --get-id --default-search ytsearch").split(" "));
    return ytProcess.stdout.toString().trim();
}

function downloadYoutube(player, url, msgChannel, verbose = false, attachRecorder) {
    if (url.startsWith("<")) url = url.substring(1);
    if (url.endsWith(">")) url = url.substring(0, url.length - 1);
    //fs.appendFileSync("./audio/yt/log.txt", "Downloading YouTube URL");
    var id = url.includes("newgrounds") ? url.split("/")[5] : (url.includes("youtube.com/watch?v=") ? url.split("=")[1] : getFirstResultID(url));
    ...
    if (fs.existsSync("./audio/yt/" + id + ".mp3")) {
        ...
        var ytProcess = cp.spawn("yt-dlp", (url + " -x --audio-format mp3 -o ./audio/yt/" + id + ".mp3").split(" "));
        ...
        ytProcess.on("close", (code) => {
            if (code == 1) {
                sendWebhook("flaps", "Error downloading youtube URL.", true, msgChannel);
            } else {
                attachRecorder(player, "yt/" + id);
            }
        });
...
async function downloadYoutubeToMP3(url, msgChannel, verbose = false, client) {
    ...
    if (fs.existsSync("./audio/yt/" + id + ".mp4")) {
        ...
        var filePath = path.join(__dirname, '../audio/yt/', id + '.mp3');
        var ytProcess = cp.spawn("yt-dlp", (url + " -x --audio-format mp3 -o " + filePath).split(" "));
        ytProcess.stdout.on("data", (data) => {
            //fs.appendFileSync("./audio/yt/log.txt", data.toString());
            if (verbose) sendWebhook("flaps", data.toString(), true, msgChannel);
        });
        ytProcess.stderr.on("data", (data) => {
            sendWebhook("flaps", data.toString(), true, msgChannel);
        });
...
function startWatchParty(url, msgChannel) {
    ...
    if (fs.existsSync("E:/MBG/2Site/sites/konalt/flaps/watchparty/videos/" + id + ".mp4")) {
        fetch("https://konalt.us.to:4930/start", {
            ...
            sendWebhook("flaps", `[yer watch party is ready](https://konalt.us.to/flaps/watchparty/?id=${response.id})\nSponsored by **FlapsCache:tm:**`, true, msgChannel);
        });
    } else {
        var filePath = "E:/MBG/2Site/sites/konalt/flaps/watchparty/videos/" + id + ".mp4";
        var thumbPath = "E:/MBG/2Site/sites/konalt/flaps/watchparty/videos/thumb_" + id + ".png";
        var ytProcess = cp.spawn("yt-dlp", (url + " -f mp4 -o " + filePath).split(" "));
        ytProcess.stdout.on("data", (data) => {
            //fs.appendFileSync("./audio/yt/log.txt", data.toString());
            if (verbose) sendWebhook("flaps", data.toString(), true, msgChannel);
        });
        ...
        ytProcess.on("close", async(code) => {
            if (code == 1) {
                ...
                download("https://img.youtube.com/vi/" + id + "/maxresdefault.jpg", thumbPath, () => {
                    fetch("https://konalt.us.to:4930/start", {
                        method: "POST",
                        body: JSON.stringify({
                            videoId: id
