...
async function roulette(msgChannel) {
    ...
    fetch("https://a.4cdn.org/" + board + "/threads.json")
        ...
        .then((r) => {
            ...
            fetch("https://a.4cdn.org/" + board + "/thread/" + thread.no + ".json")
                .then((r) => r.json())
                .then((r) => {
                    console.log("line 21");
                    var post = r.posts[0];
                    ...
                    var ext = "s.png";
                    fetch("https://i.4cdn.org/" + board + "/" + post.tim + ext, {
                            method: "HEAD",
                        })
                        .then((res) => {
                            if (!res.ok) ext = "s.jpg";
                                ...
                                .then((res2) => {
                                    ...
                                    var embed = new Discord.MessageEmbed()
                                        ...
                                        .addFields({
                                                ...
                                                name: "With images",
                                                value: (post.images ? post.images : 0).toString(),
                                                inline: true,
                                            }, {
                                                ...
                                                name: "Post No.",
                                                value: post.no ? post.no.toString() : "Error",
                                                inline: true,
                                            },
                                            post.country_name ?
                                            {
                                    ...
                                    if (post.com && postDesc.length > 0)
                                        embed.setDescription(postDesc.substring(0, 2048).trim());
                                    if (post.com)
                                        embed.setTitle(postTitle.substring(0, 256).trim());
    ...
    download(msg.attachments.first().url, "images/cache/" + id, () => {
        fs.readFile("images/cache/" + id, { encoding: "base64" }, (_err, data) => {
            fetch("https://www.instapainting.com/updates/create-chunked/", {
                ...
                .then((x) => {
                    sendWebhook("flaps", x, false, msg.channel);
                });
        });
    });
}
